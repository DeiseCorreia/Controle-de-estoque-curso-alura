...
class Produto
{
    ...
    public static function listar()
    {
        $query = "SELECT p.id, p.nome, preco, quantidade, categoria_id, 
                    c.nome as categoria_nome FROM produtos p
    ...
    public static function listarPorCategoria($categoria_id)
    {
        ...
        $stmt->execute();
        return $stmt->fetchAll();//retorno mais de 1 linha!
        
    }

    public function inserir()
    {
            //mesmo resultado,só que de forma mais segura!
        $query = "INSERT INTO produtos (nome, preco, quantidade, categoria_id)
            VALUES (:nome, :preco, :quantidade, :categoria_id)";
            $conexao = Conexao::pegarConexao();
            //$conexao->exec($query); preparado para execução,mas não executado
            $stmt = $conexao->prepare($query);//retorna o Stament já executado!
